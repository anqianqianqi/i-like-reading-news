...
const BRIEF_SCHEMA = {
  type:"object",properties:{date:{type:"string"},markets:{type:"object",properties:{summary_title:{type:"string"},key_mechanism:{type:"string"},week_ahead:{type:"string"},tickers:{type:"array",items:{type:"object",properties:{label:{type:"string"},value:{type:"string"},change:{type:"string"},direction:{type:"string",enum:["up","dn","neutral"]}},required:["label","value","change","direction"],additionalProperties:false}}},required:["summary_title","key_mechanism","week_ahead","tickers"],additionalProperties:false},stories:{type:"array",items:{type:"object",properties:{title:{type:"string"},sources:{type:"array",items:{type:"string"}},what:{type:"string"},narrative:{type:"string",description:"Storyteller version of 'what'. Lead with the human angle — who did what, what was at stake, what happened. Write like the opening of a good news article: characters, drama, stakes. Include all key facts but frame them in narrative terms. 2-3 sentences."},mechanism:{type:"array",items:{type:"object",properties:{label:{type:"string"},steps:{type:"array",items:{type:"object",properties:{text:{type:"string"},type:{type:"string",enum:["cause","mechanism","result-short","result-long"]}},required:["text","type"],additionalProperties:false}}},required:["label","steps"],additionalProperties:false}},narrative_why:{type:"string",description:"Storyteller version of the mechanism. Explain WHY it happened and what it leads to in plain connected prose — no arrows, no step labels. Use analogies when helpful. 2-3 sentences that a curious non-expert would find satisfying. Example: 'The catch most people miss: it is American companies, not Canadian ones, that pay this tariff when goods cross the border. That means US manufacturers who rely on Canadian steel suddenly face a 50% increase in their costs — and they must choose between taking the hit or passing it to consumers.'"},so_what:{type:"array",items:{type:"string"},minItems:2,maxItems:3},price_moves:{type:"array",items:{type:"object",properties:{ticker:{type:"string"},company:{type:"string"},direction:{type:"string",enum:["up","dn","watch"]},magnitude:{type:"string"},reason:{type:"string"}},required:["ticker","company","direction","magnitude","reason"],additionalProperties:false}},glossary_terms:{type:"array",description:"REQUIRED. List every finance/tech term. Do not leave empty.",items:{type:"string"}},highlights:{type:"array",items:{type:"object",properties:{text:{type:"string"},type:{type:"string",enum:["number","company","risk","positive"]}},required:["text","type"],additionalProperties:false}}},required:["title","sources","what","narrative","mechanism","narrative_why","so_what","price_moves","glossary_terms","highlights"],additionalProperties:false}},quick_hits:{type:"array",items:{type:"object",properties:{topic:{type:"string"},detail:{type:"string"},highlight:{type:"string"},source:{type:"string"}},required:["topic","detail","highlight","source"],additionalProperties:false}}},required:["date","markets","stories","quick_hits"],additionalProperties:false
};
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (!blobAvailable) {
          log("⚠ BLOB_STORE_ID not found — results won't be cached.");
        }
        if (!force) {
